...
/*
  ...
  - 7-bit I2C address for SEN6x family (SEN66): 0x6B. :contentReference[oaicite:0]{index=0}
  - Commands used:
      0x0021 Start Continuous Measurement (SEN6x). :contentReference[oaicite:1]{index=1}
      0x0202 Get Data Ready (SEN6x). :contentReference[oaicite:2]{index=2}
      0x0300 Read Measured Values (SEN66). Returns 27 bytes (9 * [MSB, LSB, CRC]) with PMs, RH, T, VOC, NOx, CO2. :contentReference[oaicite:3]{index=3}
      0x0316 Read Number Concentrations (SEN6x). Returns PM0.5..PM10 number conc. (5 * triplets). :contentReference[oaicite:4]{index=4}
      0xD206 Read Device Status (uint32 flags). :contentReference[oaicite:5]{index=5}
  - Data words are 16-bit MSB-first, each followed by CRC-8 (poly 0x31, init 0xFF). :contentReference[oaicite:6]{index=6}
*/
...
public:
    ...
    bool begin(int sda = SEN66_I2C_SDA, int scl = SEN66_I2C_SCL, uint32_t freq = SEN66_I2C_FREQ);
